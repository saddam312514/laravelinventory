    ...
    public function index()
    {
        ...
        if (request()->ajax()) {
            ...
            $purchases_returns = Transaction::leftJoin('contacts', 'transactions.contact_id', '=', 'contacts.id')
                    ->leftjoin('purchase_lines', 'purchase_lines.transaction_id', '=', 'transactions.return_parent_id')
                    ->join(
                    ...
                    ->select(
                        ...
                        DB::raw('SUM(IF(purchase_lines.quantity_returned>0,1,0)) as returned_product')
                    )
