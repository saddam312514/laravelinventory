...
class ReportController extends Controller
{
    ...
    public function getStockReport(Request $request)
    {
        ...
        if ($request->ajax()) {
            ...
            $products = $query->select(
                ...
                DB::raw("(SELECT SUM(IF(transactions.type='sell_return', TSL.quantity_returned, 0) ) FROM transactions 
                        LEFT JOIN transaction_sell_lines AS TSL ON transactions.return_parent_id=TSL.transaction_id

                        WHERE transactions.status='final' AND transactions.type='sell_return' $location_filter 
    ...
    public function getproductPurchaseReport(Request $request)
    {
        ...
        return view('report.product_purchase_report')
            ->with(compact('business_locations', 'suppliers', 'categories'));
    }
